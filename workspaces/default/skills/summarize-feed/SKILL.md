...
1. **Call the tool:** `summarize_article(article_id=<id>)`
   ...
   - Optionally pass `force: true` to regenerate the summary even if cached (see [Force Regeneration](#force-regeneration) below)
...
3. **Present the summary:**
   - Show the article title as a header (with source name)
   - Present the `summary` text **preserving its structure** — the backend returns a structured format (see [Format Rules](#format-rules)). Do NOT re-summarize or shorten what the backend returned.
...
>
> **Visão geral:** O governo federal lançou um pacote de medidas voltado à agricultura familiar, combinando novas linhas de crédito, assistência técnica e simplificação de acesso a programas existentes. O anúncio foi feito pelo Ministro do Desenvolvimento Agrário durante evento em Brasília nesta terça-feira.
>
> **Pontos principais:**
> - R$ 5 bilhões em linhas de crédito com juros subsidiados para pequenos produtores
> - Criação de 200 novos postos de assistência técnica em municípios do Nordeste
> - Simplificação do acesso ao Pronaf com redução de burocracia
> - Ampliação do programa de compras institucionais para merenda escolar
>
> **Contexto do BMO:** Essas medidas chegam em um momento em que o setor da agricultura familiar vinha cobrando maior apoio do governo, especialmente após os impactos das secas no Nordeste no último ano. O foco na assistência técnica e na desburocratização sugere que o governo está respondendo diretamente às principais demandas das associações de pequenos produtores.
>
...
`summarize_article` caches the LLM summary server-side — calling the same article again returns the cached result instantly at no cost. If the user explicitly wants a **new** summary (because the cached one is outdated, poorly written, or the article content changed), pass `force: true`:

```
summarize_article(article_id=42, force=true)
```

**When to use `force: true`:**
- User says "refaz o resumo", "atualiza o resumo", "esse resumo ficou ruim, tenta de novo"
- User says the article changed or the summary seems stale

**When NOT to use `force: true`:**
- **Never by default** — each force call costs a DeepSeek API call
- Don't use it just because the user asked for a summary of an already-cached article — the cached result is likely fine
...
- **Individual summaries (summarize_article):** Structured format generated by the backend/DeepSeek — preserve it as-is when presenting. The structure is:
  - **Visão geral:** A paragraph summarizing the article's main narrative
  - **Pontos principais:** Bullet points with the key facts/reported information
  - **Contexto do BMO:** (optional) Analysis or broader context the model adds *beyond* what the article reports — clearly separated from the reported facts. If the backend includes this section, present it; if absent, don't invent one.
- **Panorama summaries (BMO-synthesized):** Free-form — the BMO writes its own aggregated overview from `title` + `summary_raw`. This mode does NOT use the structured format above (Visão geral/Pontos principais/Contexto do BMO), because it's a loose synthesis of MANY articles, not a formal deep summary of one. 1 paragraph per source (3–5 sentences if the source has multiple articles; 1–2 sentences if only one).
